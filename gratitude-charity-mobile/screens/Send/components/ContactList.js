...
const ListItem = (item, key, self) => (
  <TouchableOpacity key={key} onPress={() => self.props.navigation.navigate('SendProfile', { selected: item })}>
...
const ContactList = (self) => {
  ...
  return (
    <SectionList
      ...
      sections={self.state.contacts.sort((a, b) => (a.title > b.title) ? 1 : -1)}
      renderItem={({ item, index, section }) => ListItem(item, `${index}-${item.name}-${item.phoneNumber}`, self)}
      keyExtractor={(item, index) => `${index}-${item.name}-${item.phoneNumber}`}
